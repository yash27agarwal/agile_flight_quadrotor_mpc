...
class Trajectory:
    ...
    def desired_altitude(self, quad:Quadrotor, idx, N_):
        ...
        dz_ref_ = np.concatenate((quad.state['velocity'][2], dz_ref_), axis=None)
        
        ddz_ref_ = np.diff(dz_ref_)
        ddz_ref_ = np.concatenate((ddz_ref_[0], ddz_ref_), axis=None)

        thrust_ref_ = (quad.gravity - ddz_ref_)*quad.mass
        
        x_ = np.array([z_ref_, dz_ref_]).T
        x_ = np.concatenate((np.array([[quad.state['position'][2], quad.state['orientation'][2]]]), x_), axis=0)
        u_ = np.array([thrust_ref_]).T
        # print(x_)
        # print(u_)
        return x_, u_

    def desired_position(self, quad:Quadrotor, idx, N_, thrust):
        # initial state / last state
        x_ = np.zeros((N_+1, 4))
        u_ = np.zeros((N_, 2))
...
def main():
    ...
    Plot.plot(his_time, his_thrust, his_tau_phi, his_tau_the, his_tau_psi, 
         quad_vel, quad.path, traj.ref, his_motor_speeds)
